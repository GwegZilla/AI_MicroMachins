...
static inline void DataTypeFormatString(ImGuiDataType data_type, void* data_ptr, const char* display_format, char* buf, int buf_size)
{
	...
	else if (data_type == ImGuiDataType_Char)
		ImFormatString(buf, buf_size, display_format, *(char*)data_ptr);
	else if (data_type == ImGuiDataType_UChar)
		ImFormatString(buf, buf_size, display_format, *(unsigned char*)data_ptr);
	else if (data_type == ImGuiDataType_String)
		ImFormatString(buf, buf_size, display_format, ((std::string*)data_ptr)->c_str());
}
...
static inline void DataTypeFormatString(ImGuiDataType data_type, void* data_ptr, int decimal_precision, char* buf, int buf_size)
{
	...
	{
		if (decimal_precision < 0)
			ImFormatString(buf, buf_size, "%d", *(char*)data_ptr);
		else
			ImFormatString(buf, buf_size, "%.*d", decimal_precision, *(char*)data_ptr);
	}
	else if (data_type == ImGuiDataType_UChar)
	{
		if (decimal_precision < 0)
			ImFormatString(buf, buf_size, "%d", *(unsigned char*)data_ptr);
		else
			ImFormatString(buf, buf_size, "%.*d", decimal_precision, *(unsigned char*)data_ptr);
	}
	else if (data_type == ImGuiDataType_String)
	{
		ImFormatString(buf, buf_size, "%s", decimal_precision, *(unsigned char*)data_ptr);
	}
    else if (data_type == ImGuiDataType_Float)
    {
        if (decimal_precision < 0)
...
static void DataTypeApplyOp(ImGuiDataType data_type, int op, void* value1, const void* value2)// Store into value1
{
	...
	{
		if (op == '+')
			*(char*)value1 = *(char*)value1 + *(const char*)value2;
		else if (op == '-')
			*(char*)value1 = *(char*)value1 - *(const char*)value2;
	}
	else if (data_type == ImGuiDataType_UChar)
	{
		if (op == '+')
			*(unsigned char*)value1 = *(unsigned char*)value1 + *(const unsigned char*)value2;
		else if (op == '-')
			*(unsigned char*)value1 = *(unsigned char*)value1 - *(const unsigned char*)value2;
	}
    else if (data_type == ImGuiDataType_Float)
...
static bool DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* data_ptr, const char* scalar_format)
{
	...
	else if (data_type == ImGuiDataType_Char)
	{
		...
		// Store operand in a float so we can use fractional value for multipliers (*1.1), but constant always parsed as integer so we can fit big integers (e.g. 2000000003) past float precision
		float arg1 = 0.0f;
		if (op == '+') { if (sscanf(buf, "%f", &arg1) == 1) *v = (char)(arg0 + arg1); }                // Add (use "+-" to subtract)
		else if (op == '*') { if (sscanf(buf, "%f", &arg1) == 1) *v = (char)(arg0 * arg1); }                // Multiply
		else if (op == '/') { if (sscanf(buf, "%f", &arg1) == 1 && arg1 != 0.0f) *v = (char)(arg0 / arg1); }// Divide
		else { if (sscanf(buf, scalar_format, &arg0) == 1) *v = arg0; }                     // Assign constant
		return (old_v != *v);
	}
	else if (data_type == ImGuiDataType_UChar)
	{
		if (!scalar_format)
			scalar_format = "%d";
		unsigned char* v = (unsigned char*)data_ptr;
		const unsigned char old_v = *v;
		int arg0 = *v;
		if (op && sscanf(initial_value_buf, scalar_format, &arg0) < 1)
			return false;
		...
		// Store operand in a float so we can use fractional value for multipliers (*1.1), but constant always parsed as integer so we can fit big integers (e.g. 2000000003) past float precision
		float arg1 = 0.0f;
		if (op == '+') { if (sscanf(buf, "%d", &arg1) == 1) *v = (unsigned char)(arg0 + arg1); }                // Add (use "+-" to subtract)
		else if (op == '*') { if (sscanf(buf, "%d", &arg1) == 1) *v = (unsigned char)(arg0 * arg1); }                // Multiply
		else if (op == '/') { if (sscanf(buf, "%d", &arg1) == 1 && arg1 != 0.0f) *v = (unsigned char)(arg0 / arg1); }// Divide
		else 
		{ 
			if (sscanf(buf, scalar_format, &arg0) == 1) *v = static_cast<unsigned char>(arg0); // Assign constant
		}                     
		return (old_v != *v);
	}
...
bool ImGui::SliderChar(const char* label, char* v, int v_min, int v_max, const char* display_format)
{
	if (!display_format)
		display_format = "%.0f";
	float v_f = (float)*v;
...
bool ImGui::SliderUchar(const char* label, unsigned char* v, int v_min, int v_max, const char* display_format)
{
	if (!display_format)
		display_format = "%.0f";
	float v_f = (float)*v;
...
bool ImGui::InputChar(const char* label, char* v, int step, int step_fast, ImGuiInputTextFlags extra_flags)
{
	// Hexadecimal input provided as a convenience but the flag name is awkward. Typically you'd use InputText() to parse your own data, if you want to handle prefixes.
	const char* scalar_format = (extra_flags & ImGuiInputTextFlags_CharsHexadecimal) ? "%08X" : "%d";
	return InputScalarEx(label, ImGuiDataType_Char, (void*)v, (void*)(step > 0.0f ? &step : NULL), (void*)(step_fast > 0.0f ? &step_fast : NULL), scalar_format, extra_flags);
}

bool ImGui::InputUChar(const char* label, unsigned char* v, int step, int step_fast, ImGuiInputTextFlags extra_flags)
{
	// Hexadecimal input provided as a convenience but the flag name is awkward. Typically you'd use InputText() to parse your own data, if you want to handle prefixes.
	const char* scalar_format = (extra_flags & ImGuiInputTextFlags_CharsHexadecimal) ? "%08X" : "%d";
	return InputScalarEx(label, ImGuiDataType_UChar, (void*)v, (void*)(step > 0.0f ? &step : NULL), (void*)(step_fast > 0.0f ? &step_fast : NULL), scalar_format, extra_flags);
}
